...
class ConfigValidator:
    ...
    def validate_plugin_parameters(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Advanced options
        validated_params['force'] = self._validate_boolean(
            params.get('force', False), 'force'
        )
        validated_params['backup'] = self._validate_boolean(
            params.get('backup', False), 'backup'
        )

        # Timeout settings
        validated_params['timeout'] = self._validate_timeout(
        ...
        # File permissions
        validated_params['file_mode'] = self._validate_file_mode(
            params.get('file_mode', '0600')
        )
    ...
    def _validate_api_key(self, api_key: Any) -> str:
        ...
        if not isinstance(api_key, str):
            raise ZeroSSLConfigurationError(
                ...
                parameter="api_key",
                parameter_value=str(type(api_key))
            )
    ...
    def _validate_state(self, state: Any) -> str:
        ...
        if not isinstance(state, str):
            raise ZeroSSLConfigurationError(
                ...
                parameter="state",
                parameter_value=str(type(state))
            )
    ...
    def _validate_csr_content(self, csr: Any) -> str:
        ...
        if not isinstance(csr, str):
            raise ZeroSSLConfigurationError(
                "csr must be a string",
                parameter="csr",
                parameter_value=str(type(csr))
            )

        csr = csr.strip()
        if not csr:
            raise ZeroSSLConfigurationError(
                "csr content cannot be empty",
                parameter="csr"
            )

        # Basic CSR format validation
        if not ('-----BEGIN CERTIFICATE REQUEST-----' in csr and
                '-----END CERTIFICATE REQUEST-----' in csr):
            raise ZeroSSLConfigurationError(
    ...
    def _validate_parameter_compatibility(self, params: Dict[str, Any]):
        ...
        if (validation_method == 'HTTP_CSR_HASH' and
            params['state'] in ['present', 'validate'] and
            'web_root' not in params):
            raise ZeroSSLConfigurationError(
        ...
        if 'csr_path' in params and 'csr' in params:
            raise ZeroSSLConfigurationError(
                "csr_path and csr parameters are mutually exclusive"
            )
    ...
    def get_parameter_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'optional': {
                ...
                'ca_bundle_path': {
                    'type': 'path',
                    'description': 'Path to save CA bundle file'
                },
                'full_chain_path': {
                    'type': 'path',
                ...
                'csr_path': {
                    'type': 'path',
                    'description': 'Path to existing CSR file'
                },
                'csr': {
                    'type': 'str',

...
class TestZeroSSLCertificateCreationContractImproved:
    ...
    def test_create_certificate_request_format_validation(self, mock_http_boundary, sample_api_key, sample_domains, sample_csr):
    ...
    def test_create_certificate_response_processing(self, mock_http_boundary, sample_api_key):
            ...
            "validation": {
                ...
                "other_methods": {
                    ...
                    "dev.example.com": {
                        ...
                        "file_validation_content": ["E5F6G7H8I9J0K1L2", "comodoca.com", "M3N4O5P6Q7R8S9T0"]
                    }
                }
            }
        }
        ...
        result = client.create_certificate(domains, "-----BEGIN CERTIFICATE REQUEST-----\nMULTI_DOMAIN_CSR\n-----END CERTIFICATE REQUEST-----")
        ...
        for domain in domains:
            ...
            assert domain_validation['file_validation_url_http'].startswith(f'http://{domain}/.well-known/pki-validation/')
    ...
    def test_create_certificate_api_error_handling(self, mock_http_boundary, sample_api_key):
        ...
        error_scenarios = [
            (400, {"error": {"code": 400, "type": "invalid_request", "info": "Invalid CSR format"}}),
            (401, {"error": {"code": 401, "type": "unauthorized", "info": "Invalid API key"}}),
            (403, {"error": {"code": 403, "type": "forbidden", "info": "API access denied"}}),
            (429, {"error": {"code": 429, "type": "rate_limit_exceeded", "info": "Rate limit exceeded"}}),
            (500, {"error": {"code": 500, "type": "internal_error", "info": "ZeroSSL server error"}})
        ]
    ...
    def test_create_certificate_parameter_validation(self, sample_api_key):
        ...
        validation_scenarios = [
            ...
            (["invalid_domain_!@#"], "valid_csr", "must have at least two labels", ZeroSSLConfigurationError),
...
class TestZeroSSLCertificateRetrievalContractImproved:
    ...
    def test_download_certificate_format_processing(self, mocker, sample_api_key):
        ...
        fake_zip_content = b'PK\x03\x04\x14\x00\x00\x00\x08\x00certificate.crt content\nca_bundle.crt content'
        mock_response = Mock()
...
class TestZeroSSLValidationContractImproved:
    ...
    def test_validate_certificate_request_processing(self, mock_http_boundary, sample_api_key):
        ...
        mock_http_boundary(f'/certificates/{certificate_id}/challenges', validation_response, status_code=200)
    ...
    def test_validation_methods_compatibility(self, mock_http_boundary, sample_api_key):
        ...
        for method in validation_methods:
            ...
            mock_http_boundary(f'/certificates/{certificate_id}/challenges', method_response, status_code=200)
    ...
    def test_validation_error_scenarios(self, mock_http_boundary, sample_api_key):
        ...
        validation_error_scenarios = [
            (400, {"error": {"type": "validation_failed", "info": "Domain not accessible for HTTP validation"}}),
            (422, {"error": {"type": "invalid_method", "info": "Validation method not supported for this certificate"}}),
            (409, {"error": {"type": "already_validated", "info": "Certificate has already been validated"}}),
            (404, {"error": {"type": "certificate_not_found", "info": "Certificate not found or expired"}})
        ]

        for status_code, error_response in validation_error_scenarios:
            # Mock only HTTP boundary - simulate ZeroSSL validation error
            mock_http_boundary(f'/certificates/{certificate_id}/challenges', error_response, status_code=status_code)
    ...
    def test_validation_status_monitoring(self, mock_http_boundary, sample_api_key):
        ...
        for i, status_response in enumerate(validation_statuses):
            mock_http_boundary(f'/certificates/{certificate_id}/status', status_response, status_code=200)

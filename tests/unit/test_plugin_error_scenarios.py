...
class TestActionModuleErrorScenarios:
    ...
    def test_unexpected_exception_handling(
        self, mock_ansible_environment, mocker, mock_http_boundary
    ):
        ...
        mocker.patch.object(
            action_module, "_validate_parameters", side_effect=RuntimeError("Unexpected error")
        )

        with pytest.raises(AnsibleActionFail):
            action_module.run(task_vars=mock_ansible_environment.task_vars)

    def test_api_client_creation_failure(
        self, mock_ansible_environment, mocker, mock_http_boundary
    ):
        ...
        mocker.patch.object(
            action_module,
            "_create_api_client",
            side_effect=ZeroSSLHTTPError("API client creation failed"),
        )
    ...
    def test_certificate_manager_creation_failure(
        self, mock_ansible_environment, mocker, mock_http_boundary
    ):
        ...
        mocker.patch.object(
            action_module,
            "_create_certificate_manager",
            side_effect=ZeroSSLConfigurationError("Certificate manager creation failed"),
        )
    ...
    def test_validation_handler_creation_failure(
        self, mock_ansible_environment, mocker, mock_http_boundary
    ):
        ...
        mocker.patch.object(
            action_module,
            "_create_validation_handler",
            side_effect=ZeroSSLValidationError("Validation handler creation failed"),
        )
    ...
    def test_csr_file_reading_error(
        self, mock_ansible_environment, temp_directory, mock_http_boundary
    ):
    ...
    def test_backup_file_creation_error(
        self, mock_ansible_environment, temp_directory, mocker, mock_http_boundary
    ):
    ...
    def test_file_permission_error(
        ...
        mock_http_boundary("/certificates", {"id": "permission_cert_123", "status": "draft"})

        mock_http_boundary(
            "/certificates/permission_cert_123/download/return/zip",

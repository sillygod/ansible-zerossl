...
class TestZeroSSLAPIClientImproved:
    ...
    def test_api_client_initialization_real(self, sample_api_key):
        ...
        custom_client = ZeroSSLAPIClient(
            api_key=sample_api_key, base_url="https://custom.zerossl.com", max_retries=5, timeout=60
        )
    ...
    def test_create_certificate_with_real_api_logic(
        self, mock_http_boundary, real_api_client, sample_domains, sample_csr
    ):
        """
        ...
        result = real_api_client.create_certificate(
            domains=sample_domains, csr=sample_csr, validity_days=90
        )
    ...
    def test_real_error_handling_and_exception_types(
        self, mock_http_boundary, real_api_client, sample_domains, sample_csr
    ):
        """
        ...
        mock_http_boundary(
            "/certificates", ERROR_RATE_LIMIT, status_code=429, headers=RATE_LIMIT_EXCEEDED_HEADERS
        )
        ...
        mock_http_boundary(
            "/certificates/nonexistent", ERROR_CERTIFICATE_NOT_FOUND, status_code=404
        )
    ...
    def test_rate_limit_header_processing_real(self, real_api_client):
        ...
        with unittest.mock.patch("requests.Session.get", return_value=mock_resp):
            ...
            assert real_api_client.rate_limit_remaining == int(
    ...
    def test_api_performance_within_limits(
        self, mock_http_boundary, real_api_client, sample_domains, sample_csr
    ):
        """
    ...
    def test_contract_compliance_method_signatures(
        self, real_api_client, sample_domains, sample_csr
    ):
        """
    ...
    def test_real_http_method_routing(
        self, mock_http_boundary, real_api_client, sample_domains, sample_csr
    ):
        """
        ...
        assert (
            "POST" in http_methods_used
        )  # create_certificate, validate_certificate, cancel_certificate
    ...
    def test_cancel_certificate_real_method(self, mock_http_boundary, real_api_client):
        ...
        mock_http_boundary(
            f"/certificates/{certificate_id}/cancel",
            {"success": True, "message": "Certificate cancelled successfully"},
        )

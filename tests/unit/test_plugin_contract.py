...
class TestAnsiblePluginInterfaceContractImproved:
    ...
    def test_plugin_validates_required_parameters(self, mock_ansible_environment):
        ...
        test_cases = [
            ...
            (
                {"api_key": "test_key_A1B2C3D4E5F6G7H8I9J0K1L2M3N4O5P6", "state": "present"},
                "domains",
            ),  # Missing domains
        ]
    ...
    def test_plugin_validates_parameter_types_and_constraints(self, mock_ansible_environment):
        ...
        for invalid_args, problematic_param in invalid_scenarios:
            ...
            if problematic_param == "domains" and "should_be_list" in str(
                invalid_args.get("domains", "")
            ):
    ...
    def test_plugin_return_structure_present_state(
        self, mock_ansible_environment, mocker, temp_directory
    ):
    ...
    def test_plugin_return_structure_request_state(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
    ...
    def test_plugin_return_structure_check_renewal_state(
        self, mock_ansible_environment, mock_http_boundary
    ):
        ...
        mock_http_boundary(
            "/certificates/zerossl_cert_E5F6G7H8I9J0K1L2",
            certificate_status_response,
            status_code=200,
        )
    ...
    def test_plugin_idempotent_behavior(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
        ...
        mock_http_boundary("/certificates", certificate_list_response, status_code=200)
        mock_http_boundary(
            "/certificates/zerossl_cert_F6G7H8I9J0K1L2M3",
            certificate_status_response,
            status_code=200,
        )
        ...
        mock_http_boundary(
            "/certificates/zerossl_cert_F6G7H8I9J0K1L2M3/download", zip_buffer.getvalue()
        )
...
class TestAnsiblePluginErrorHandlingContractImproved:
    ...
    def test_plugin_handles_api_errors_gracefully(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
        ...
        api_error_scenarios = [
            (
                401,
                {"error": {"code": 401, "type": "unauthorized", "info": "Invalid API key"}},
                "unauthorized",
            ),
            (
                403,
                {"error": {"code": 403, "type": "forbidden", "info": "Access denied"}},
                "forbidden",
            ),
            (
                429,
                {"error": {"code": 429, "type": "rate_limited", "info": "Rate limit exceeded"}},
                "rate limit",
            ),
            (
                500,
                {"error": {"code": 500, "type": "internal_error", "info": "Server error"}},
                "server error",
            ),
        ]
    ...
    def test_plugin_handles_network_errors_with_retry_info(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
        ...
        assert any(
            ...
            for keyword in ["timeout", "connection", "failed", "not found", "api request"]
        )
...
class TestAnsiblePluginDocumentationContractImproved:
    ...
    def test_documentation_parameter_consistency(self):
        ...
        for param_name, expected_attrs in critical_params.items():
            assert (
                param_name in documented_options
            ), f"Parameter {param_name} missing from documentation"
            ...
            if expected_attrs["required"]:
                assert (
                    param_doc.get("required") is True
                ), f"{param_name} should be marked as required"
...
class TestAnsiblePluginStateContractImproved:
    ...
    def test_all_documented_states_implemented(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
        ...
        for state in supported_states:
            ...
            if state == "absent":
                ...
                mock_http_boundary(
                    "/certificates/zerossl_cert_STATE_TEST_123456/cancel", {"success": True}
                )
                ...
                if state in ["validate", "download", "present"]:
                    mock_http_boundary(
                        "/certificates/zerossl_cert_STATE_TEST_123456/challenges", {"success": True}
                    )
                    mock_http_boundary(
                        "/certificates/zerossl_cert_STATE_TEST_123456",
                        {
                            "id": "zerossl_cert_STATE_TEST_123456",
                            "status": "issued",
                            "common_name": "state-test.example.com",
                    ...
                    mock_http_boundary(
                        "/certificates/zerossl_cert_STATE_TEST_123456/download",
                        zip_buffer.getvalue(),
                    )
    ...
    def test_invalid_state_error_handling(self, mock_ansible_environment):
        ...
        for invalid_state in invalid_states:
            ...
            assert any(
                valid_state in result.get("msg", "")
                for valid_state in ["present", "request", "validate"]
            )

    def test_action_module_run_method_execution(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
        """
    ...
    def test_certificate_renewal_flow_with_valid_cert(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
    ...
    def test_certificate_files_need_update_scenario(
        self, mock_ansible_environment, mock_http_boundary, temp_directory
    ):
        ...
        mock_http_boundary(
            "/certificates/valid_cert_456/download/return/zip",
            {"certificate.crt": "-----BEGIN CERTIFICATE-----\nNEW_CERT\n-----END CERTIFICATE-----"},
        )
    ...
    def test_file_backup_creation(
        ...
        mock_http_boundary(
            "/certificates",
            {"id": "backup_cert_789", "status": "draft", "common_name": "backup.example.com"},
        )
    ...
    def test_download_state_handling(self, mock_ansible_environment, mock_http_boundary):
        ...
        mock_http_boundary(
            "/certificates/download_cert_ghi/download/return/zip",
            {
                "certificate.crt": "-----BEGIN CERTIFICATE-----\nDOWNLOAD_CERT\n-----END CERTIFICATE-----"
            },
        )

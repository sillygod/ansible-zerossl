...
-----END CERTIFICATE REQUEST-----"""
        ...
        if result.get("failed"):
            ...
            assert any(
                ...
                for keyword in ["unauthorized", "invalid", "api key", "auth"]
            )
        else:
...
-----END CERTIFICATE REQUEST-----"""
    ...
    def test_certificate_download_failure(
        self,
        mock_action_base,
        mock_task_vars,
        sample_api_key,
...
-----END CERTIFICATE REQUEST-----"""
        ...
        if result.get("failed"):
            ...
            assert any(
                keyword in error_message
                for keyword in ["download", "certificate", "not found", "failed"]
            )
        else:
...
-----END CERTIFICATE REQUEST-----"""
        ...
        if result.get("failed"):
            error_message = result.get("msg", "").lower()
            assert any(
                keyword in error_message for keyword in ["json", "parse", "invalid", "response"]
            )
        else:
...
-----END CERTIFICATE REQUEST-----"""
    ...
    def test_ssl_verification_error(
        self,
        mock_action_base,
        mock_task_vars,
        sample_api_key,
...
-----END CERTIFICATE REQUEST-----"""
        ...
        if result.get("failed"):
            ...
            assert any(
                keyword in error_message
                for keyword in ["ssl", "certificate", "verification", "failed"]
            )
        else:

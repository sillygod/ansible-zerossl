...
-----END CERTIFICATE REQUEST-----"""
    ...
    def test_private_key_file_permissions(self, mock_action_base, mock_task_vars,
                                        sample_api_key, sample_domains, temp_directory,
                                        mock_http_boundary, mock_zerossl_api_responses):
...
-----END CERTIFICATE REQUEST-----"""
    ...
    def test_validation_file_security(self, mock_action_base, mock_task_vars,
                                    sample_api_key, sample_domains, temp_directory,
                                    mock_http_boundary, mock_zerossl_api_responses):
...
-----END CERTIFICATE REQUEST-----"""
    ...
    def test_error_message_sanitization(self, mock_action_base, mock_task_vars,
                                      sample_api_key, sample_domains, temp_directory,
                                      mock_http_boundary, mock_zerossl_api_responses):
...
-----END CERTIFICATE REQUEST-----"""
        ...
        if result.get('failed'):
            ...
            assert sample_api_key not in error_message, "API key should not appear in error messages"
            assert len(error_message) > 0, "Error message should not be empty"
            assert any(keyword in error_message.lower() for keyword in ['error', 'failed', 'invalid', 'validation', 'required']), \
                "Error message should contain meaningful error information"
        else:
            # If not failed, check that error handling was graceful
            assert 'changed' in result

    def test_secure_file_handling_edge_cases(self, mock_action_base, mock_task_vars,
                                           sample_api_key, sample_domains, temp_directory,
                                           mock_http_boundary, mock_zerossl_api_responses):
...
-----END CERTIFICATE REQUEST-----"""
    ...
    def test_memory_security_sensitive_data(self, mock_action_base, mock_task_vars,
                                          sample_api_key, sample_domains, temp_directory,
                                          mock_http_boundary, mock_zerossl_api_responses):
            ...
            for attr_name in dir(action_module):
                if not attr_name.startswith('_'):
                    ...
                    if isinstance(attr_value, str):
                        assert sample_api_key not in attr_value, f"API key found in attribute {attr_name}"
                    elif isinstance(attr_value, dict):
                        attr_str = str(attr_value)
                        assert sample_api_key not in attr_str, f"API key found in dict attribute {attr_name}"

    def test_input_validation_security(self, mock_action_base, mock_task_vars,
                                     sample_api_key, sample_domains, temp_directory,
                                     mock_http_boundary, mock_zerossl_api_responses):
...
-----END CERTIFICATE REQUEST-----"""
            ...
            try:
                ...
                if dangerous_file.exists() and dangerous_file.is_absolute():
                    ...
                    if str(dangerous_path).startswith('/etc/') or str(dangerous_path).startswith('/root/'):
                ...
                error_message = str(e)
                assert sample_api_key not in error_message, "API key should not be in validation error messages"

    def test_certificate_chain_validation_security(self, mock_action_base, mock_task_vars,
                                                  sample_api_key, sample_domains, temp_directory,
                                                  mock_http_boundary, mock_zerossl_api_responses):

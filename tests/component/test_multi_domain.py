...
-----END CERTIFICATE REQUEST-----"""
        ...
        if result.get('failed'):
            ...
            error_message = result.get('msg', '').lower()
            assert any(keyword in error_message for keyword in ['validation', 'failed', 'domain', 'error'])
        else:
            # If it succeeds, validation was handled gracefully
            assert 'changed' in result

    def test_san_certificate_performance_with_many_domains(self, mock_action_base, mock_task_vars,
                                                         sample_api_key, temp_directory,
                                                         mock_http_boundary, mock_zerossl_api_responses):

...
class MockZeroSSLAPIClient:
    ...
    def create_certificate(self, domains, csr, validity_days=90):
        ...
        # Add validation data
        cert_response['validation'] = {
            'other_methods': create_validation_files_response(domains)
        }
    ...
    def get_verification_details(self, certificate_id):
        ...
        return {
            'validation': {
                'other_methods': create_validation_files_response(['example.com'])
            }
        }
...
class MockValidationHandler:
    ...
    def place_validation_files(self, validation_files, web_root):
        ...
        return {
            'success': True,
            'files_placed': len(validation_files),
            'web_root': web_root
        }
    ...
    def side_effect(*args, **kwargs):
        ...
        if '/certificates' in url and 'download' in url:
            ...
            response = MockResponse(
                content=create_mock_certificate_zip(),
                status_code=200,
            ...
            response = MockResponse(
                json_data=responses.get('validate', {}),
                status_code=200,
                headers=RATE_LIMIT_HEADERS
            )
            ...
            # Default response
            response = MockResponse(
                json_data=responses.get('default', {}),
                status_code=200,
                headers=RATE_LIMIT_HEADERS
            )
...
def assert_file_permissions(file_path, expected_mode):
    ...
    assert actual_mode == expected_str, f"File permissions mismatch: expected {expected_str}, got {actual_mode}"
